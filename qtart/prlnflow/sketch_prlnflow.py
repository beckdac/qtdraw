...
class LogoSketch(vsketch.SketchClass):
    ...
    noise_threshold = vsketch.Param(value=.9, min_value=0, max_value=1, step=.01, decimals=2)
        ...
        for x in np.arange(x_min + self.freq, x_max, self.freq*2):
            for y in np.arange(y_min+self.freq, y_max, self.freq*2):
                ...
                if noise < self.noise_threshold:
                    print(f"skipping particle starting at {x} {y} for low noise {noise}")
                    continue
                ...
                while p.position[0] >= x_min and p.position[0] <= x_max and \
                    ...
                    if all(np.isclose(p.velocity, 0, 4)):
                        print(f"particle starting at {x} {y} died at {p.position[0]}, {p.position[1]}")
                        break
                print(f"particle starting at {x} {y} died or left at {p.position[0]}, {p.position[1]}")
